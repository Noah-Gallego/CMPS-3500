...
def load_data(file_path, label):
    ...
    print(f"{timestamp()} Total Columns Read: {df.shape[1]}")
    print(f"{timestamp()} Total Rows Read: {df.shape[0]}")
...
def process_data():
    ...
    print(f"{timestamp()} Total data shape: {df.shape}")
    print(f"{timestamp()} Train indicators: {df['__origin'].value_counts().get(0, 0)}")
    print(f"{timestamp()} Test indicators: {df['__origin'].value_counts().get(1, 0)}")
...
def predict(df):
    ...
    try:
        print("\n*********************")
        print(f"{timestamp()} Evaluating models with the test data")
        ...
        if potential_leakage:
            if '__origin' in df.columns:
                print(f"\n{timestamp()} Removing __origin column used for data splitting")
        ...
        print(f"\n{timestamp()} Training data shape: {train_data.shape}")
        print(f"{timestamp()} Testing data shape: {test_data.shape}")
        ...
        print(f"{timestamp()} Precision: {precision:.4f}")
        print(f"{timestamp()} Recall: {recall:.4f}")
        print(f"{timestamp()} F1 Score: {f1:.4f}")
        print(f"{timestamp()} ROC AUC: {auc:.4f}")
        ...
        for idx in random_indices:
            ...
            print(f"{timestamp()} Sample #{sample_idx}:")
            print(f"{timestamp()} True: {true_label}")
            print(f"{timestamp()} Predicted: {pred_label} (Confidence: {prob:.4f})")
...
def accuracy():
    ...
    print(f"{timestamp()} Accuracy: {metrics['accuracy']:.4f}")
    print(f"{timestamp()} Precision: {metrics['precision']:.4f}")
    print(f"{timestamp()} Recall: {metrics['recall']:.4f}")
    print(f"{timestamp()} F1 Score: {metrics['f1']:.4f}")
    print(f"{timestamp()} ROC AUC: {metrics['auc']:.4f}")
...
def menu():
    ...
    while True:
        ...
        if choice == '1':
            filename = input("Enter training CSV filename (e.g., LA_Crime_Data_2023_to_Present_data.csv): ").strip()
            file_path = os.path.join("Data", "Dirty", filename)
            ...
            filename = input("Enter testing CSV filename (e.g., LA_Crime_Data_2023_to_Present_test1.csv): ").strip()
            file_path = os.path.join("Data", "Dirty", filename)
